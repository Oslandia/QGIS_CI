...
void Qgs3DBoundingBoxEntity::createLabels( Qt::Axis axis, const QgsVector3D &bboxMin, const QgsVector3D &bboxMax, float maxExtent, QList<QVector3D> &vertices )
{
  ...
  {
    case Qt::Axis::XAxis:
      nrIncrD = std::max( ( int ) std::lround( nrTicks * ( bboxMax.x() - bboxMin.x() ) / maxExtent ), 3 );
      QgsBoundingBoxUtils::extendedWilkinsonAlgorithm( bboxMin.x(), bboxMax.x(), nrIncrD, false, rangeMin, rangeMax, rangeIncr );

      coordTmp = rangeMin - mMapSettings->origin().x();
      incr = QVector3D( rangeIncr, 0.0f, 0.0f );
      pt1 = QVector3D( coordTmp, bbox.yMin, bbox.zMin );
      pt2 = QVector3D( coordTmp, bbox.yMin, bbox.zMax );
      pt3 = QVector3D( coordTmp, bbox.yMax, bbox.zMin );
    ...
    case Qt::Axis::YAxis:
      ...
      // QgsBoundingBoxUtils::extendedWilkinsonAlgorithm( bboxMin.y(), bboxMax.y(), nrIncrD, false, rangeMin, rangeMax, rangeIncr );
      QgsBoundingBoxUtils::extendedWilkinsonAlgorithm( bboxMin.z(), bboxMax.z(), nrIncrD, false, rangeMin, rangeMax, rangeIncr );
    ...
    case Qt::Axis::ZAxis:
      ...
      coordTmp = - ( rangeMax - mMapSettings->origin().y() );
