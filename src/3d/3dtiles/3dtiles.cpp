...
Q3dCube::Q3dCube(const QgsVector3D &ll, const QgsVector3D &ur) :
		Q3dPrimitive() {
	/*mPoints.append(QgsVector4D(ll, 1.0));
	 mPoints.append(QgsVector4D(ur, 1.0));*/
...
QVector<QgsPoint> Q3dCube::asQgsPoints() {
	...
	for (int i = 0; i < mPoints.size(); i++) {
//      cubePts.append(QgsPoint(mPoints[i][0]-4000000.0, mPoints[i][1], mPoints[i][2]-4910000.0));
...
BoundingVolume::BoundingVolume(const BoundingVolume::BVType &t) :
		mType(t), mEpsg(QgsCoordinateReferenceSystem::fromEpsgId(4979)) {
	// noop
}
...
QgsAABB BoundingVolume::asQgsAABB(const QgsMatrix4x4 & transform, const QgsCoordinateTransform * coordTrans) {
    Q3dCube cube = asCube(transform, coordTrans);
    return QgsAABB(cube.mPoints[0].x(), cube.mPoints[0].y(), cube.mPoints[0].z(),
            cube.mPoints[1].x(), cube.mPoints[1].y(), cube.mPoints[1].z());
}
...
//  mCenter[0] = mCenter[0]/1000.0;
//  mCenter[1] = mCenter[1]/1000.0;
//  mCenter[2] = mCenter[2] - 4910000;
//  mW *= 10.0;
    QgsMatrix4x4 rotMat = fromRotationTranslation();
    qDebug() << "BOX rot matrix: " << rotMat << "\n";
    cube = transform * rotMat * cube;
    if (coordTrans) {
        cube.reproject(*coordTrans);
    }
    qDebug() << "BOX cube: " << cube << "\n";
    return cube;
}
...
// =======================
Tile::Tile() :
        mParentTileset(NULL), mBv(), mCombinedTransform(NULL) {
	// noop
}
...
Tile::Tile(const QJsonObject &obj, int depth, Tileset *parTs, Tile *par) :
        mParentTileset(parTs), mParentTile(par), mCombinedTransform(NULL) {
    ...
    if (obj.find("content") != obj.end() ) {
        mContent.setFrom(obj["content"].toObject(), depth, this);
	}
...
QgsMatrix4x4 Tile::getCombinedTransformRec(Tile *p) {
    if (p->mParentTile != NULL) {
        return getCombinedTransformRec(p->mParentTile) * mTransform;
    } else {
        if (p->mParentTileset != NULL) {
            return getCombinedTransformRec(p->mParentTileset) * mTransform;
        } else {
        return mTransform;
}
    }
...
QgsAABB Tile::getBoundingVolumeAsAABB(
    ...
    return mBv->asQgsAABB(*ct, coordTrans2);
}
...
const TileContent& TileContent::setFrom(const QJsonObject &obj, int depth,
	...
	if (mUrl.toString().endsWith(".json")) {
		...
		mSubContent = Tileset::fromUrl(mUrl,
                mParentTile->mParentTileset->mSource, parentTile, depth + 1);
...
Tile * Tileset::findTile(const QgsChunkNodeId & tileId, const QgsCoordinateTransform *coordTrans){
    ...
    if (tileId.d == 0 ) {
        ...
        QgsVector3D tileCenter(tileId.x, tileId.y, tileId.z);
        out = findTileRecInTileset(this, 0, tileCenter, coordTrans);
    }
    ...
    if (curTile->mChildren.isEmpty()) { // no children
        ...
        for (std::shared_ptr<Tile> child : curTile->mChildren) {
            if (child->contains(tileCenter, coordTrans)) {
                out = findTileRecInTile(curTile, depth +1, tileCenter, coordTrans);
                break;
            }
        }
    }
...
bool Tile::contains(const QgsVector3D &point, const QgsCoordinateTransform *coordTrans) {
    QgsMatrix4x4 * ct = getCombinedTransform();
    Q3dCube c = mBv->asCube(*ct, coordTrans);
    return false;
}
