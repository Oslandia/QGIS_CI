...
void QgsTessellator::addPolygon( const QgsPolygon &polygon, float extrusionHeight )
{
  ...
  const float radsBetweenUpNormal = qAcos( pNormalUpVectorDotProduct );

  const float detectionDelta = qDegreesToRadians( 10.0f );
  int facade = 0;
  if ( radsBetweenUpNormal > M_PI_2 - detectionDelta && radsBetweenUpNormal < M_PI_2 + detectionDelta ) facade = 1;
  else if ( radsBetweenUpNormal > - M_PI_2 - detectionDelta && radsBetweenUpNormal < -M_PI_2 + detectionDelta ) facade = 1;
  else facade = 2;
  ...
  if ( pCount == 4 && polygon.numInteriorRings() == 0 && ( mTessellatedFacade & facade ) )
  {
    ...
    for ( int i = 0; i < 3; i++ )
    {
      const double baseHeight = !zData || mNoZ ? 0 : *zData;
      const double z = mNoZ ? 0 : ( baseHeight + extrusionHeight );
      if ( baseHeight < zMin )
        zMin = baseHeight;
